...
TrayIconsSearch(sTerm="") {
	...
	Tray_Icons := TrayIconsOverflow(sTerm)
...
TrayIcons(sExeName = "") {
	return _getTrayIcons( "Shell_TrayWnd", sExeName )
}

TrayIconsOverflow(sExeName = "") {
	return _getTrayIcons( "NotifyIconOverflowWindow", sExeName )
}
...
_getTrayIcons( toolBarClass, execNameOrPid = "" ) {
	...
	WinGet,	pidTaskbar, PID, ahk_class %toolBarClass%
	hProc:=	DllCall("OpenProcess", "Uint", 0x38, "int", 0, "Uint", pidTaskbar)
	pProc:=	DllCall("VirtualAllocEx", "Uint", hProc, "Uint", 0, "Uint", 32, "Uint", 0x1000, "Uint", 0x4)

	if( toolBarClass == "Shell_TrayWnd" ) {
		toolBarIndex := GetTrayBar()
	} else {
		toolBarIndex := 1
	}

	SendMessage, 0x418, 0, 0, ToolbarWindow32%toolBarIndex%, ahk_class %toolBarClass%   ; TB_BUTTONCOUNT

	Loop,	%ErrorLevel%
	{
		SendMessage, 0x417, A_Index-1, pProc, ToolbarWindow32%toolBarIndex%, ahk_class %toolBarClass%   ; TB_GETBUTTON
		...
		If !execNameOrPidToSearch || (execNameOrPidToSearch = sProcess) || (execNameOrPidToSearch = pid)
